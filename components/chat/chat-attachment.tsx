      ...
      <div className="overflow-hidden rounded-xl">
        ...
        >
          ...
          <img
            ...
            className={cn("max-h-64 max-w-full object-cover transition-opacity", imgLoading ? "hidden" : "block", "hover:opacity-90")}
            onLoad={() => setImgLoading(false)}
            onError={() => {
              setImgError(true);
              setImgLoading(false);
    ...
    <div className="relative flex w-28 flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-sm">
      <div className="relative h-20 w-full overflow-hidden bg-void-900/30">
        ...
        <button
          type="button"
          onClick={onRemove}
          className="absolute right-1.5 top-1.5 flex h-8 w-8 items-center justify-center rounded-full bg-void-900/80 text-white backdrop-blur hover:bg-red-500"
