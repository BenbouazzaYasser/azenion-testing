...
interface BranchDeleteTarget {
  dbId: string;
  /** Short display name / acronym (branches.name). */
  shortName: string;
  /* Full institution name (branches.full_name). */
  name: string;
}

interface BranchDeleteDialogProps {
  branch: BranchDeleteTarget;
  onClose: () => void;
}
...
export function BranchDeleteDialog({ branch, onClose }: BranchDeleteDialogProps) {
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Delete ${branch.shortName}`}
    >
      <button
        type="button"
        aria-label="Close"
      ...
      <div
        ref={dialogFocusRef}
        tabIndex={-1}
        className="relative z-10 flex max-h-[85vh] w-full max-w-[440px] flex-col overflow-hidden rounded-2xl panel-gradient shadow-dialog backdrop-blur-2xl transition-all duration-200 ease-premium focus:outline-none"
        style={{
          opacity: mounted ? 1 : 0,
      ...
      >
        <div className="flex items-start justify-between border-b border-border px-8 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-ink-50">Delete Branch</h2>
              <p className="mt-1 text-sm text-ink-400">{branch.shortName}</p>
            </div>
          </div>
          <button
            type="button"
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            <div className="flex flex-col gap-2">
              <p className="text-sm font-medium text-ink-50">
                Delete {branch.shortName}?{branch.name && branch.name !== branch.shortName ? ` (${branch.name})` : ""}
              </p>
              <p className="text-sm text-ink-400">
                This permanently removes the branch and all of its assets. A branch with
                remaining members cannot be deleted.
              </p>
            </div>
            <input
              value={confirm}
              onChange={(e) => setConfirm(e.target.value)}
              placeholder={`Type ${branch.shortName} to confirm`}
              aria-label="Type branch name to confirm deletion"
        ...
        <div className="flex items-center justify-end gap-3 border-t border-border px-8 py-5">
          ...
          <Button
            ...
            disabled={confirm !== branch.shortName || isPending}
