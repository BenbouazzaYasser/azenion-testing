...
export async function ProjectPageRecruitment({ roles }: ProjectPageRecruitmentProps) {
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-3 text-sm text-ink-500">
                          <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium ${EXPERIENCE_COLORS[role.experience] || EXPERIENCE_COLORS.intermediate}`}>
                            <Target size={10} />
                            {await serverT(EXPERIENCE_LABELS[role.experience] || "projects.expIntermediate")}
                          </span>
                          <span className="inline-flex items-center gap-1">
                            <Users size={12} className="text-accent-400" />
                            {role.positions} {role.positions === 1 ? await serverT("projects.positionOne") : await serverT("projects.positionMany")}
                          </span>
                        </div>
                      </div>
                    </div>
