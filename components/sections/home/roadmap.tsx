    ...
    <section className="relative py-20 sm:py-24 lg:py-28" aria-labelledby="roadmap-heading">
      <div className="mx-auto max-w-[1320px] px-5 sm:px-8 lg:px-12">
        ...
        <ol className="relative mx-auto mt-16 max-w-3xl">
          ...
          {(await Promise.all(ROADMAP.map(async (item, i) => {
            ...
            return (
              <li key={item.titleKey} className="relative pb-10 last:pb-0">
                  ...
                  <div className="flex gap-5">
                    ...
                    <div className="min-w-0 flex-1 rounded-2xl card-surface-soft p-5 shadow-card backdrop-blur-xl transition-all duration-500 ease-premium hover:border-accent-400/30 sm:p-6">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <h3 className="text-lg font-semibold text-ink-50">{await serverT(item.titleKey)}</h3>
                        <span
                          className={cn(
                            "inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-normal",
                            item.status === "done"
                        ...
                        >
                          ...
                          {await serverT(STATUS_LABEL_KEY[item.status])}
                        </span>
                      </div>
          ...
          <p className="mx-auto mt-12 flex max-w-3xl items-center justify-center gap-2 text-center text-sm text-ink-500">
            ...
            {await serverT("home.roadmapNote")}
          </p>
