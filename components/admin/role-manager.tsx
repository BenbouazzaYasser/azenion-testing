...
export function RoleManager({ roleCatalog }: RoleManagerProps) {
  ...
  return (
    <div className="space-y-6">
      {/* ── User search ─────────────────────────────────────────────── */}
      <section className="rounded-2xl bg-surface shadow-card backdrop-blur-xl">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            runSearch(query);
        ...
        >
          <input
            ...
            className="flex-1 rounded-xl bg-surface px-4 py-3 text-[0.95rem] text-ink-50 placeholder:text-ink-600 outline-none transition-colors focus:border-accent-400/60 focus:bg-surface-hover"
          />
                ...
                <ul className="flex flex-wrap gap-2">
                  {userRoles.map((assignment) => {
                    ...
                    return (
                      <li
                        key={assignment.name}
                        className="inline-flex items-center gap-2 rounded-full bg-surface px-4 py-1.5 text-xs font-medium tracking-wide text-ink-200"
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                  <div className="flex-1">
                    ...
                    <select
                      ...
                      className="w-full appearance-none rounded-xl bg-surface px-4 py-3 text-[0.95rem] text-ink-50 outline-none transition-colors focus:border-accent-400/60 focus:bg-surface-hover"
                    >
