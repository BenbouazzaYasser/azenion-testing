      ...
      <SettingsPanel>
        <div className="flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7">
          <div className="flex items-center gap-4">
            <div className="h-16 w-16 shrink-0 overflow-hidden rounded-2xl border border-white/[0.12] bg-gradient-to-br from-accent/[0.15] to-accent/[0.05]">
              {profile.avatar_url ? (
            ...
            {profile.skills.length ? (
              <div className="flex flex-wrap gap-1.5">
                {profile.skills.slice(0, 6).map((skill) => (
                  <span key={skill} className="rounded-full border border-white/[0.08] bg-white/[0.04] px-2 py-0.5 text-[11px] text-ink-200">
                    {skill}
                  </span>
                ))}
                {profile.skills.length > 6 ? (
                  <span className="rounded-full border border-white/[0.08] bg-white/[0.04] px-2 py-0.5 text-[11px] text-ink-400">
                    +{profile.skills.length - 6}
                  </span>
                ) : null}
              </div>
        ...
        <SettingsPanel>
          <SummaryRow icon={<Link2 size={16} className="text-accent-300" />} label="Social links">
            <div className="flex gap-3 text-ink-300">
              {profile.github_url ? (
                <a href={profile.github_url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] px-2 py-1 text-xs hover:text-ink-50" aria-label="GitHub profile">
              ...
              {profile.linkedin_url ? (
                <a href={profile.linkedin_url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] px-2 py-1 text-xs hover:text-ink-50" aria-label="LinkedIn profile">
