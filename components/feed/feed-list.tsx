          ...
          <div className="flex flex-col items-center justify-center gap-4 px-6 py-20 text-center">
            ...
            {filter !== "all" ? (
              <button
                ...
                className="mt-1 rounded-full border border-border-strong/[0.08] bg-surface px-6 py-2.5 text-sm font-medium text-ink-200 shadow-card backdrop-blur-xl transition-all duration-300 ease-premium hover:-translate-y-0.5 hover:border-accent-400/40 hover:text-ink-50 hover:shadow-glow-sm"
              >
                Show all posts
              </button>
            ) : null}
        ...
        {isLoadingMore && (
          <div className="flex flex-col gap-4" aria-live="polite" aria-busy="true">
            {Array.from({ length: 2 }).map((_, i) => (
              <div
                key={i}
                className="animate-pulse rounded-2xl border border-border-strong/[0.08] bg-surface p-5 shadow-card backdrop-blur-xl"
              >
